...
# - Sector -
conditionalPanel(
  condition = "input.ambito_series == 'Regiones'",
  selectInput(
    inputId = "sector_series",
    label = "Sector:",
    choices = NULL, # Lo rellenamos dinámicamente desde el server
    selected = NULL
  )
)
...
conditionalPanel(
  condition = "input.ambito_series == 'Sectores laborales'",
  selectInput(
    inputId = "seleccion_series",
    label = "Selecciona región:",
    choices = NULL, # Lo rellenamos desde el server
    selected = NULL
  )
)
...
checkboxInput(
  "scale_free_y", "Escala eje Y libre", value = TRUE)
...
# --- Seleccionar region solo disponible para comparar por sectores --- 
observe({
  # Solo trabajamos si estamos en la pestaña correcta y hay un NUTS seleccionado
  req(input$ambito_series, input$nuts_series)
  
  # Si no estamos en modo sectores, no gastamos recursos calculando
  if (input$ambito_series != "Sectores laborales") return()

  # 1. Filtramos las opciones disponibles según el NUTS
  df_opciones <- data %>%
    select(full_name, geo, NUTS) %>%
    distinct() # Nos quedamos con los únicos para no procesar todo el dataset

  if (input$nuts_series == "CCAA") {
    # Caso especial CCAA: NUTS 2 y empieza por ES
    df_opciones <- df_opciones %>% filter(NUTS == 2, grepl("^ES", geo))
  } else {
    # Caso normal (0, 1, 2)
    df_opciones <- df_opciones %>% filter(NUTS == input$nuts_series)
  }

  # 2. Creamos el vector de opciones (Nombre = Codigo)
  opciones_vector <- df_opciones %>%
    arrange(full_name) %>%
    tibble::deframe()

  # 3. Lógica de Prioridad (Default)
  #    Definimos a quién buscamos
  target_name <- if (input$nuts_series == "0") "Spain" else "Canarias"
  
  #    Buscamos el código de ese target en las opciones disponibles
  #    (Usamos grepl para ser flexibles si se llama "Canarias (ES)" o similar)
  codigo_por_defecto <- df_opciones %>%
    filter(grepl(target_name, full_name, ignore.case = TRUE)) %>%
    pull(geo) %>%
    head(1) # Por seguridad cogemos el primero si hubiera varios

  # 4. Selección Final
  #    Si encontramos al target, lo seleccionamos. Si no (ej: datos corruptos), el primero de la lista.
  seleccion_final <- if (length(codigo_por_defecto) > 0) codigo_por_defecto else opciones_vector[1]

  # 5. Actualizamos el input
  updateSelectInput(
    session = session, # Asegúrate de tener 'session' en tu función server(input, output, session)
    inputId = "seleccion_series",
    label = if(input$nuts_series == "0") "Selecciona país:" else "Selecciona región:",
    choices = opciones_vector,
    selected = seleccion_final
  )
})

# --- Filtrar indicador con datos por sectores si vamos a comparar por ellos ---
observe({
  req(input$ambito_series)
  
  if (input$ambito_series == "Sectores laborales") {
    
    # OPCIÓN A: MODO SECTORES
    # Le pasamos UNA sola opción en 'choices'. Así el usuario no puede elegir otra.
    updateSelectInput(
      inputId = "indicador_series",
      choices = c("Coste laboral / hora trabajada" = "D1_SAL_HW"), 
      selected = "D1_SAL_HW"
    )
    
  } else {
    
    # OPCIÓN B: MODO REGIONES
    # Le devolvemos la lista completa original 'indicadores_labels'
    updateSelectInput(
      inputId = "indicador_series",
      choices = indicadores_labels, 
      selected = indicadores_labels[2]
    )
    
  }
})

# --- Solo sectores disponibles ---
observe({
  req(input$indicador_series, input$ambito_series)
  
  # Solo trabajamos si estamos en la pestaña Regiones
  if (input$ambito_series == "Regiones") {
    
    # ¿Es el indicador que tiene sectores? (Usa el código interno, ej: "D1_SAL_HW")
    if (input$indicador_series == "D1_SAL_HW") {
      
      # Opción A: Cargamos TODOS los sectores
      lista_sectores <- c("Total actividades", sort(setdiff(unique(data$sector_name), "Total actividades")))
      
      updateSelectInput(
        inputId = "sector_series",
        choices = lista_sectores,
        selected = "Total actividades" # O input$sector_series para mantener selección
      )
      
    } else {
      
      # Opción B: Forzamos a SOLO Total Actividades
      updateSelectInput(
        inputId = "sector_series",
        choices = c("Total actividades"),
...
filtrado_series_temporales <- reactive({
  ...
  if (input$nuts_series == "CCAA") {
    ...
    # Caso CCAA: Filtramos NUTS 2 Y solo regiones de España (empiezan por ES)
    df_base <- df_base %>%
      filter(substr(geo, 1, 2) == "ES" & NUTS == 2)
      
  } else {
    
    # Caso Normal (0, 1, 2): Filtramos tal cual
    df_base <- df_base %>%
      filter(NUTS == input$nuts_series)
  ...
  if (input$ambito_series == "Regiones") { 
    ...
    df_base <- df_base %>% filter(sector_name == input$sector_series)
    df_fijos <- df_fijos %>% filter(sector_name == input$sector_series)
    ...
    if (n_total > 0) {
      ...
      geo_seleccionados <- na.omit(lista_geo[indice_inicio:indice_fin])
    }
...
renderPlotly({
  ...
  if (input$ambito_series == "Regiones") {
    ...
    p <- tb %>% 
      ...
      geom_line() +
      facet_wrap(~full_name, scales = scale_y, ncol=3) +
      theme_minimal() +
    ...
    req(input$seleccion_series)
    
    p <- tb %>% 
      filter(geo == input$seleccion_series) %>%
      ggplot(aes(year, y=values)) +
      geom_line() +
      facet_wrap(~sector_name, scales = scale_y, ncol=4) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust=1)) + 
      labs(x="", y="")
...
```{r widgets-superpuestos-series}
div(
  # Estilo del contenedor general (barra blanca superior)
  style = "display: flex; align-items: center; gap: 40px; height: 80px; padding: 0 20px; background-color: white; position: relative; z-index: 50;", 
  
  # --- WIDGET 1: REFERENCIA ---
  div(
    # 'display: flex' aquí pone el TEXTO y el SELECT uno al lado del otro
    style = "display: flex; align-items: center; gap: 10px;", 
    
    tags$strong("Referencia:", style = "white-space: nowrap;"), # Etiqueta manual
    
    selectInput(
      inputId = "ref_superpuestos",
      label = NULL, # <--- ¡IMPORTANTE! Lo ponemos NULL para que no salga arriba
      choices = c("España" = "ES", "Canarias" = "ES7", "Media UE" = "EU27_2020"),
      selected = "EU27_2020",
      width = "160px" # Ancho controlado del cajetín
    )
  ),
  
  # --- WIDGET 2: REGIONES (Múltiple) ---
  div (
    # 'flex-grow: 1' hace que este ocupe todo el espacio sobrante a la derecha
    style = "display: flex; align-items: center; gap: 10px; flex-grow: 1;",
    
    tags$strong("Regiones:", style = "white-space: nowrap;"), # Etiqueta manual
    
    selectInput(
      inputId = "seleccion_multi_superpuestos",
      label = NULL, # <--- ¡IMPORTANTE! NULL aquí también
      choices = NULL,
      selected = c("Spain", "Italy", "France", "Germany"),
      multiple = TRUE,
...
# --- Regiones según el NUTS seleccionado ---
observe({
  req(input$nuts_series, input$ambito_series)
  
  # 1. Filtramos las opciones disponibles según el NUTS seleccionado en el sidebar
  df_opciones <- data %>%
    select(full_name, geo, NUTS) %>%
    distinct()

  if (input$nuts_series == "CCAA") {
    df_opciones <- df_opciones %>% filter(substr(geo, 1, 2) == "ES" & NUTS == 2)
  } else {
    df_opciones <- df_opciones %>% filter(NUTS == input$nuts_series)
  }

  # 2. Convertimos a vector
  opciones_vector <- df_opciones %>%
    arrange(full_name) %>%
    tibble::deframe()
  
  # grep devuelve la posición donde encuentra el texto.
  idx_spain    <- grep("Spain", names(opciones_vector), ignore.case = TRUE)
  idx_canarias <- grep("Canarias", names(opciones_vector), ignore.case = TRUE)
  
  # Lógica de prioridad:
  seleccion_defecto <- if (length(idx_spain) > 0) {
      # Si existe Spain, cogemos su código
      opciones_vector[idx_spain[1]] 
  } else if (length(idx_canarias) > 0) {
      # Si no, si existe Canarias, cogemos su código
      opciones_vector[idx_canarias[1]]
  } else {
      # Si no hay ninguno, NULL (o opciones_vector[1] si quieres forzar el primero)
      NULL 
  }
  
  # 3. Actualizamos el selector múltiple
  updateSelectInput(
    session = session,
    inputId = "seleccion_multi_superpuestos",
    choices = opciones_vector,
...
tags$div(
  ...
  highcharter::highchartOutput("grafico_series_out", height = "100%")
)

output$grafico_series_out <- renderHighchart({
  # 1. Validaciones básicas
  req(
    input$indicador_series, 
    input$unidad_series, 
    input$seleccion_multi_superpuestos,
    input$ref_superpuestos # Aseguramos que haya referencia seleccionada
  )
  
  # 2. Datos de las REGIONES SELECCIONADAS (Líneas de colores)
  data_vars <- data %>%
    filter(
      item == input$indicador_series,
      unit == input$unidad_series,
      sector_name == input$sector_series,
      geo %in% input$seleccion_multi_superpuestos, # Filtramos las regiones elegidas
      !is.na(values)
    ) %>%
    arrange(year)

  # 3. Datos de la REFERENCIA (Línea negra discontinua)
  #    Aquí buscamos directamente el código (ES, ES70, EU27_2020, etc.)
  data_ref <- data %>%
    filter(
      item == input$indicador_series,
      unit == input$unidad_series,
      sector_name == input$sector_series,
      geo == input$ref_superpuestos, # Aquí entra "EU27_2020" directo
      !is.na(values)
    ) %>%
    arrange(year)
  ...
  #    Añadimos la serie de Referencia (si existe)
  if (nrow(data_ref) > 0) {
    
    # Sacamos el nombre bonito para la leyenda (ej: "European Union - 27 countries")
    nombre_ref <- head(unique(data_ref$full_name), 1)
    ...
    hc <- hc %>% 
      hc_add_series(
        ...
        name = paste(nombre_ref, "(Ref)"), # Etiqueta en leyenda
        dashStyle = "LongDash",   # Estilo discontinuo
        color = "lightgray",
        marker = list(enabled = FALSE),
        zIndex = 10               # Para que se pinte siempre ENCIMA
      )
  }
  hc
})
