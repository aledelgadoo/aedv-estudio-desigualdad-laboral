...
pickerInput(
    ...
    options = pickerOptions(
        actionsBox = TRUE,
        liveSearch = TRUE,
        noneSelectedText = "Ninguna CCAA seleccionada" # Texto que se muestra cuando está vacío
    )
)
```
...
noUiSliderInput(
    ...
    value = 2000,
    step = 1,
    # color = "#007bff", # Opcional: color azulito
    format = wNumbFormat(decimals = 0) # Para que no salgan decimales (2010.00)
)
...
observe({
  ...
  años_disponibles <- data %>%
    filter(
      ...
      unit == input$unidad_arima,
      sector_name == input$sector_arima,
      # Aquí usamos el ID (geo) que devuelve el pickerInput
      geo == input$region_arima, 
      !is.na(values)
    ) %>%
    pull(year) %>%
  ...
  updateNoUiSliderInput(
    session = session,
    inputId = "year_arima",
    range = c(min_year, max_year), # Nuevos límites
...
renderPlotly({
  ...
  p <- ggplot(datos_ajustados, aes(x = year)) +
    ...
    labs(
      title = paste("Ajuste del modelo -", input$region_arima),
      y = input$unidad_arima, x = "Año", color = NULL
    ) +
