...
selectInput(
  "x", 
  label = "x: atributo 1",
  choices = unique(population$`Sexo/Indicador`), 
  selected = "Mujeres/Var.Anual"
)
...
selectInput(
  ...
  label = "Año",
  choices = c("Todos",sort(unique(population$Año), decreasing = TRUE)), 
  selected = "Todos"
)
...
inputs.processing.comparacion.atributos <- reactive( {   
  tb <-  population.wider %>% 
  select(Año,municipio=Territorio,x=input$x,y=input$y) %>%  
  na.omit()
...
renderPlotly({
  ...
  tb <- inputs.processing.comparacion.atributos()[[1]] %>%
    mutate(`municipio/año`=paste0(municipio," / ",Año))
  
  # dibujamos el diagrama de dispersión 
  tb %>%
    ggplot(aes(x,y,label=`municipio/año`)) + 
    geom_point() +
