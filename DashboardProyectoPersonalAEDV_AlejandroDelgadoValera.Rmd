...
<!-- FUNCIÓN reactive QUE REALIZA EL PROCESADO DE LA TABLA PARA ESTA PÁGINA --> 
...
inputs.processing.mapa0 <- reactive({
  tb <- geoj0.tb %>% 
    left_join(
      data %>%
        filter(year==input$year_mapa, item==input$indicador_mapa, unit==input$unidad_mapa, sector_name==input$sector_mapa),
      join_by("NUTS_ID"=="geo")
    ) %>% 
    mutate(full_name = coalesce(full_name, NAME_LATN),
           # crear etiqueta legible combinando indicador y unidad
            indicador_unidad = paste(
            names(indicadores_labels)[match(item, indicadores_labels)],  # nombre legible del indicador
            "-",
            names(unidad_labels)[match(unit, unidad_labels)]             # nombre legible de la unidad
          )
    ) %>% 
    # Rellenar NA en indicador_unidad usando los valores existentes por item+unit
    mutate(
      indicador_unidad = if_else(
        indicador_unidad == "NA - NA",
        first(na.omit(indicador_unidad)),  # toma el primer valor no NA del grupo
        indicador_unidad
      )
    )
...
```{r}
# Este reactive "conmuta" los datos según la pestaña activa
data_para_tabla_sidebar <- reactive({
  
  # Requerimos que la pestaña tenga un valor (evita errores al inicio)
  req(input$pestana_nuts_activa) 
  
  # Usamos switch() para seleccionar el conjunto de datos correcto
  switch(input$pestana_nuts_activa,
         "NUTS 0" = inputs.processing.mapa0(),
         "NUTS 1" = inputs.processing.mapa1(),
         "NUTS 2" = inputs.processing.mapa2()
  )
})
```
...
renderTable({
 ...
 tb <- data_para_tabla_sidebar() %>%
   arrange(desc(values))%>%  
   select(full_name,values)
  
 tb %>% 
 mutate(value=prettyNum(round(values,
...
Column {.tabset data-id="pestana_nuts_activa"}
--------------------------------------------------
...
etiquetas <-paste(
 ...
 # dibujamos el mapa 
 MapaCoroplético(geoj0,tb$values,etiquetas,"Valor")
})
```
...
etiquetas <-paste(
 ...
 # dibujamos el mapa 
 MapaCoroplético(geoj1,tb$values,etiquetas,"Valor")
})
```
...
etiquetas <-paste(
 ...
 # dibujamos el mapa 
 MapaCoroplético(geoj2,tb$values,etiquetas,"Valor")
})
```
