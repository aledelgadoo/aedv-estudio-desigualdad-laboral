...
Series temporales
=================
```{r analisis-data-seriesTemporales-paises}
data_series_paises <- data %>% 
    filter(sector == "TOTAL")
```
...
div(
  ...
  conditionalPanel(
    ...
    # Comprueba si el <input> 'ID_DE_TUS_PESTAÑAS' tiene el valor 'Comparativa CCAA'
...
div(renderHighchart({
    ...
    # 1. Datos de las CCAA (del input)
      data_paises <- data_series_paises %>%
        filter(
...
div(renderHighchart({
    ...
    # 1. Datos de las CCAA (del input)
      ...
      data_espana <- data_series_paises %>%
        filter(
          full_name == "Spain", 
          item == input$indicador,
          unit == input$unidad
