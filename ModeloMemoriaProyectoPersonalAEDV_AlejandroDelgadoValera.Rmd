...
La economía europea se construyó sobre la promesa de la convergencia: la idea de que, con el tiempo y la integración de mercados, los niveles de vida y la riqueza de los estados miembros tenderían a igualarse. Sin embargo, tres décadas después, la realidad dibuja un mapa de velocidades divergentes. En este escenario, España, y muy particularmente la región ultraperiférica de Canarias, se enfrenta a una paradoja persistente: a pesar de los esfuerzos de modernización y la integración en el mercado común, la brecha salarial con el núcleo económico europeo no solo no se cierra, sino que en muchos aspectos amenaza con cronificarse.  
...
La discusión económica sobre la disparidad de riqueza en Europa gira tradicionalmente en torno a dos conceptos clave que fundamentan este estudio: la Productividad Laboral (valor añadido generado por unidad de trabajo) y el Coste Laboral Unitario (la relación entre el coste de la mano de obra y la productividad). Históricamente, la literatura económica ha señalado una brecha persistente entre el "Norte" y el "Sur" de Europa. Mientras que los países centrales han basado su crecimiento en el aumento de la productividad total de los factores (tecnología y capital humano), estudios precedentes sobre la economía española sugieren que su modelo de crecimiento ha sido extensivo, basado en la acumulación de factores (más horas, más empleo) pero con una eficiencia estancada.  
...
La relevancia de abordar este tema ahora reside en el contexto inflacionario actual. Con los precios al alza, entender si los bajos salarios en regiones como Canarias se deben a una baja productividad real o a una estructura sectorial ineficiente es crucial. La justificación del estudio es clara: existe una necesidad urgente de validar empíricamente si la "especialización turística" y de servicios de bajo valor añadido actúa como un lastre estructural que impide la convergencia real con Europa.
...
La motivación principal es confrontar con datos una realidad que condiciona mi futuro inmediato: la narrativa generalizada de que, para prosperar en el sector tecnológico, es obligatorio emigrar a Madrid o al resto de Europa. He elegido analizar los costes laborales y la estructura sectorial para verificar empíricamente si esa "sentencia" es real. Necesito entender si la devaluación salarial en Canarias afecta también a perfiles de alta cualificación como el mío, o si es un problema exclusivo de otros sectores. En definitiva, este trabajo es una auditoría personal para descubrir si la estructura económica de las islas penaliza mi talento y me empuja inevitablemente a hacer las maletas.
...
4. Identificar el determinante sectorial: Desglosar el coste laboral por sectores económicos para demostrar cómo la composición del tejido productivo (peso de la hostelería vs. servicios profesionales) condiciona el techo salarial de cada región.  
...
Tras completar la limpieza, normalización y enriquecimiento del dataset, dejamos atrás la ingeniería de datos para adentrarnos en el análisis exploratorio. Nuestro objetivo en esta primera fase no es solo describir qué pasa, sino entender por qué España ocupa su posición actual.  

Para diagnosticar la situación salarial en Canarias y España, primero necesitamos elevar la mirada y observar el contexto macroeconómico. Europa actúa aquí como un espejo de contrastes: nos permite comparar nuestro desempeño no contra un ideal teórico, sino contra la realidad de nuestros vecinos.  
...
A través de las siguientes visualizaciones, estableceremos la "Tesis de la Eficiencia": demostraremos que existe una correlación negativa entre el presencialismo (horas trabajadas) y la productividad (coste por hora), y ubicaremos a España en este tablero de juego antes de hacer zoom en sus regiones.  
<br>
...
Esta visualización es la piedra angular del capítulo. Al cruzar el coste laboral por hora (eje Y) con las horas trabajadas por empleado al año (eje X), emerge una clara correlación negativa: los países se alinean en una pendiente descendente. Las naciones más eficientes, como Luxemburgo o Dinamarca, se agrupan en la esquina superior izquierda (alto valor, pocas horas), mientras que a medida que nos desplazamos hacia la derecha (más horas), el valor generado cae. España (la esfera morada en la zona inferior) queda relegada al cuadrante de "baja eficiencia": muchas horas de trabajo para un retorno por hora bajo. Además, la animación temporal revela cómo el COVID-19 provocó un salto generalizado hacia la izquierda en 2020, una anomalía de reducción forzosa de horas que no debe confundirse con un aumento estructural de la productividad.  
...
Si el diagrama de cajas nos sugería un problema general, este mapa de calor confirma que la desventaja es estructural y relativa. Para este análisis, hemos utilizado la variable `PC_EU27_2020_MEUR_CP`, que normaliza los datos como un porcentaje respecto al total de la UE27. Esto es fundamental porque elimina el sesgo de la riqueza absoluta: no estamos viendo simplemente qué países son más ricos, sino cómo rinde cada sector en comparación con el estándar europeo.  
El resultado es un degradado vertical implacable: las columnas de la izquierda (Luxemburgo, Dinamarca) muestran tonos oscuros intensos, indicando que sus sectores rinden muy por encima de la media europea. A medida que la vista recorre hacia la derecha, el color se desvanece hasta la palidez de España e Italia. El insight más doloroso de este gráfico es que desmonta la idea de que "basta con cambiar de sector". Incluso si comparamos filas idénticas, como "Información y comunicación" o "Profesionales y científicas" (sectores tradicionalmente de alto valor), vemos que en España tienen una intensidad de color muy inferior a sus homólogos del norte. No es solo que tengamos sectores "peores", es que nuestros sectores "buenos" generan y retribuyen mucho menos valor que los mismos sectores en Europa.  
...
```{verbatim mapa-coropletico-costeES}
...
```
```{verbatim mapa-coropletico-horasES}
geoj_esp <- geoj2[substr(geoj2$NUTS_ID, 1, 2) == "ES", ]
geoj_esp.tb <- geoj_esp %>% as_tibble()
...
Confirmado que trabajar más no nos hace más ricos, miramos la estructura de los salarios. Este diagrama de cajas nos revela la "calidad" de la desigualdad.  
- La Oportunidad en el Centro: La caja de Madrid (izquierda, verde) es alta y con numerosos outliers disparándose hacia arriba. Aunque hay salarios bajos, existe un "ascensor" claro hacia sueldos muy altos (>40€/hora).  
- El Techo Canario: En el extremo derecho, Canarias (rojo) narra una historia de compresión. No solo su mediana es la más baja, sino que su caja es compacta y carece de esos outliers superiores.  

El drama de Canarias no es solo tener una media baja; es el "techo de cristal regional". La estructura económica comprime los salarios: incluso los trabajadores de élite en las islas apenas alcanzan los niveles medios de Madrid. No hay dispersión hacia la riqueza.  
...
Para entender el problema, primero miremos la estructura base de España. Este gráfico por secciones muestra cómo se reparte la masa salarial del país. A primera vista, vemos sectores potentes: Finanzas lidera con un 11.5%, seguido de la Administración Pública.  
Sin embargo, si sumamos los sectores tradicionalmente asociados a menor valor añadido o menor escalabilidad tecnológica —Comercio y transporte (6.51%), Comercio y alojamiento (6.2%) y Construcción (6.68%)—, obtenemos un bloque masivo de la economía (casi el 20% del coste laboral total) que depende de la presencialidad física y tiene márgenes más ajustados. España no es una economía puramente tecnológica; tiene un "lastre" estructural importante en servicios de bajo valor. Pero, ¿se reparte este lastre por igual en todo el territorio?  

<br>
Aquí es donde la media nacional vuelve a engañarnos. Ese 11.5% de riqueza financiera no está esparcido uniformemente como la mantequilla; está concentrado en puntos muy específicos. Vamos a cruzar el sector con la región para ver la "radiografía" completa.
...
Hemos viajado desde los datos macroeconómicos de Europa hasta las entrañas de la estructura sectorial española. Todas las pruebas apuntan a una conclusión: la baja remuneración no es un castigo divino ni geográfico, sino la consecuencia matemática de una especialización en actividades de bajo valor.  
Ahora, aplicamos todo lo aprendido al paciente cero: Canarias. Si nuestra tesis es correcta, el archipiélago no debería ser simplemente "más pobre", sino que debería mostrar los síntomas agudos de una estructura enferma: una brecha creciente con el continente y una incapacidad sistémica para generar valor, incluso trabajando las mismas horas.  
...
p_brecha_final <- df_brecha_wide %>%
  ...
  # Labels y Estilo
  labs(
    title = "Evolución de la Brecha Salarial: Coste por Empleado (España vs. Canarias)",
...
Para entender la magnitud del problema, primero debemos mirar la película completa. Este gráfico de área no muestra simplemente salarios, muestra la distancia entre ser empleado en la Península (azul) y serlo en las Islas (rojo).  

Lo dramático de esta imagen es la tendencia. A principios de los 2000, las líneas caminaban relativamente cerca. Sin embargo, tras la crisis de 2008, se produce una ruptura: España recupera ritmo mientras Canarias se aplana. La "boca" del gráfico se abre cada vez más, convirtiendo lo que era una diferencia regional en un abismo estructural. Hoy, la brecha es la más grande de la serie histórica. Canarias no está convergiendo con el resto del país; se está desacoplando económicamente.  

<br>
Sabemos que ganamos menos y que cada vez nos alejamos más. Pero, ¿nos alejamos en todo? ¿O es que hay sectores donde "perdemos más" que en otros?
...
Este gráfico de mancuernas es la prueba pericial más detallada. Cada línea gris representa la distancia salarial en un sector específico entre España (punto azul) y Canarias (punto rojo).  

El diagnóstico es severo: Canarias pierde en todo. El punto rojo está sistemáticamente a la izquierda en todos los sectores. Pero el matiz crucial está en la longitud de la línea.  

- En sectores de bajo valor como "Agricultura" o "Artes y otros servicios", los puntos están relativamente cerca. Somos igual de "baratos" que el resto.

- Sin embargo, si nos fijamos en los sectores que deberían tirar de la economía: "Información y comunicación" o "Industria", observamos que las líneas grises son larguísimas. Esto revela una "penalización insular del talento": un ingeniero o un informático en Canarias sufre una depreciación salarial mucho mayor que un camarero comparado con sus homólogos peninsulares.

<br>
Esta imagen congela el estado actual de la brecha, pero no nos dice cómo hemos llegado hasta aquí. ¿Es esta distancia estructural algo estático, o se está ensanchando con el tiempo?
...
Si desglosamos la evolución temporal sector por sector, confirmamos la tragedia del talento. Fijaos en el gráfico de "Comercio y alojamiento" (Hostelería). Las líneas azul y roja avanzan casi pegadas, entrelazándose; el sector turístico canario es competitivo en costes con el nacional.

Ahora mirad "Profesionales y científicas" o "Información y comunicación". La línea azul (España) despega hacia arriba con fuerza, mientras la roja (Canarias) se queda rezagada, incapaz de seguir el ritmo de crecimiento de valor. No es que Canarias no crezca, es que sus sectores de alto valor están atrofiados y no logran generar la tracción que sí generan en la península.

<br>
Si nuestros sectores de alto valor pagan mucho peor que en la península, y nuestros sectores de bajo valor pagan "normal" (bajo)... ¿Qué pasa cuando la inmensa mayoría de la población trabaja en los segundos?  
...
Esta última imagen resume el destino económico de las islas y nos deja un titular preocupante: en Canarias, tener una profesión "intelectual" no garantiza un sueldo digno.

A la derecha (barras azules), vemos los Top 4 sectores que logran superar la media nacional (línea roja): Finanzas, la Administración Pública (tanto en Educación como en Salud) e Información y Comunicación. Estos actúan como "islas de riqueza", pero dependen en gran medida del empleo público o de nichos muy específicos.  

La verdadera tragedia se revela en el bloque de la izquierda (Bottom 4). Junto a sectores tradicionalmente precarios como la Agricultura o el Arte, encontramos a las Inmobiliarias y, lo más alarmante, a las Actividades Profesionales y Científicas. Este es el dato que rompe el esquema: un sector que en el resto de Europa es motor de clase media-alta, en Canarias ha caído al vagón de cola, con salarios que apenas compiten con el sector primario. Esto confirma que la economía canaria no solo paga mal en sectores precarios nacionalmente; deprecia sistemáticamente el valor del conocimiento técnico, empujando al talento local a emigrar o a aceptar condiciones muy por debajo de su cualificación.  

La conclusión del proyecto es visualmente ineludible: Canarias tiene una economía polarizada donde la "clase media" sectorial ha desaparecido. Tenemos unos pocos sectores de élite desconectados y una inmensa base de servicios de bajo valor que ancla el promedio hacia el fondo.  
