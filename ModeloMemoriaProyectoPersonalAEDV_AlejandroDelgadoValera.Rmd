...
/* H3: subsección (derivado del estilo de H2 pero más suave) */
h3 {
  font-size: 1.25em;
  color: #333;
  font-style: italic;
...
/* H4: apartado pequeño (sutil pero diferenciado) */
h4 {
  font-size: 1.1em;
  color: #444;
  font-weight: bold;
...
show_values = TRUE # flat para controlar si se imprimen los valores en las celdas
  ...
  plot_ly(
    ...
    layout(
      title = "Correlación de sectores laborales - CCAA de España",
...
- Impacto Alto: El Núcleo del Estudio (ODS 8 y 10)
Estos objetivos representan el corazón de la tesis del proyecto:
...
- Impacto Medio: Consecuencias Directas (ODS 1, 3 y 9)
Estos objetivos se ven afectados como consecuencia directa de los hallazgos económicos:
...
- Impacto Bajo/Transversal: Contexto y Transparencia
...
<!-- desarrollo de esta fase -->
Este capítulo constituye la auditoría técnica del proyecto. A diferencia del resto de apartados, centrados en la narrativa visual y el análisis económico, esta sección se ciñe estrictamente al modelo de validación metodológica establecido en las directrices académicas de la asignatura.

Su objetivo es documentar la integridad, calidad y estructura de los datos brutos (Eurostat) antes de cualquier manipulación. Por tanto, se presenta con un enfoque puramente instrumental y técnico, incluyendo la ejecución directa de código y los tests de validación necesarios para certificar que la "materia prima" cumple con los requisitos de calidad para el análisis posterior.
...
<!-- desarrollo de esta fase -->
Este capítulo actúa como el puente operativo del proyecto. Comenzamos con el **procesado técnico inicial** (limpieza, normalización y cruce de fuentes Eurostat/INE) para construir un dataset analítico robusto. Una vez garantizada la calidad del dato, el capítulo transiciona hacia la **narrativa visual**: el desarrollo secuencial de los 5 actos definidos en los objetivos, donde transformamos las tablas procesadas en la historia económica de la brecha salarial.
...
p_dispersion_estatica <- tb_comparacion_estatica %>%
  ...
  geom_point(aes(text = paste("<b>", full_name, "</b>",
                             "<br>Año:", year,
...
```{r matriz-dispersion, fig.asp=0.95}
data %>% 
